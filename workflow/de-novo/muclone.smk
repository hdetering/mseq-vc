...
rule muclone_perf_mm2clust:
  input:  mm="sim/mm.csv"
  output: tsv="muclone_perf/clusters.tsv"
  threads: 1
  run:
    import pandas as pd
...
rule muclone_perf_yaml:
  input:
    rc=["sim/%s.vars.csv" % x for x in SAMPLES]
  output:
    yml=["muclone_perf/%s.yaml" % x for x in SAMPLES]
  threads: 1
  run:
    import yaml
    for i in range(len(input.rc)):
...
rule muclone_perf_cluster2phi:
  input:
    clus="muclone_perf/clusters.tsv",
    prev="sim/clone_prev.csv"
  output:
    ["muclone_perf/tmp/%s_cluster2Phi.tsv" % x for x in SAMPLES]
  threads: 1
  run:
    import pandas as pd
    # read clusters
    ...
    for id_sample, prow in prev.iterrows():
      ...
      with open("muclone_perf/tmp/%s_cluster2Phi.tsv" % id_sample, 'wt') as f_out:
        for elements in out:
          f_out.write('\t'.join([str(e) for e in elements]) + '\n')

rule muclone_perf_conf:
  input:
    yaml=["muclone_perf/%s.yaml" % x for x in SAMPLES],
    tsv=["muclone_perf/tmp/%s_cluster2Phi.tsv" % x for x in SAMPLES]
  output:
    cfg="muclone_perf/config.yaml"
  run:
    import os, yaml
    cfg = {
      'working_dir': os.path.join(os.getcwd(), 'muclone_perf'),
      'trace_dir'  : 'tmp',
      'samples'    : { x: {
        'mutations_file': "%s.yaml" % x,
        'flat_cluster_files': "tmp/%s_cluster2Phi.tsv" % x,
...
rule muclone_perf:
  input:
    ...
    cfg="muclone_perf/config.yaml",
    vcf="sim/somatic.vcf"
  output: "muclone_perf.vcf"
  log:    "log/muclone_perf.log"
  params:
    #dir="data/{replicate}"
  threads: 1
  shell:
    ...
    if [[ -d "muclone_perf/tmp/trace" ]]; then
      for f in muclone_perf/tmp/trace/*/MuClone-sample-results.tsv; do
        awk -f {config[scripts]}/muclone2vcf.awk {input.vcf} $f > {output}
      done
    fi
